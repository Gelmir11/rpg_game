...
export class ShopScene extends Phaser.Scene {
  ...
  drawShop() {
    ...
    const shopTab = this.add.text(320, 22, '🏪 Dükkan', {
      fontSize: '16px', fontFamily: 'Arial, sans-serif', color: '#DAA520',
    ...
    this.add.text(400, 46, `Altın: ${formatGold(ps.gold)}`, {
      fontSize: '16px', fontFamily: 'Arial, sans-serif', color: '#FFD700', fontStyle: 'bold'
    }).setOrigin(0.5);
    ...
    this.add.text(195, 68, 'Envanterin (Tıkla → Sat)', {
      fontSize: '13px', fontFamily: 'Arial, sans-serif', color: '#cc99ff', fontStyle: 'bold'
    }).setOrigin(0.5);
    ...
    shopGridEntries.forEach(p => {
      ...
      this.add.text(ix, iy + p.gh * cellS / 2 - 2, formatGold(sellPrice), {
        fontSize: '8px', fontFamily: 'Arial, sans-serif', color: '#90ee90', stroke: '#000', strokeThickness: 1
      }).setOrigin(0.5, 1);
    ...
    this.add.text(600, 68, 'Dükkan (Kategori tıkla → aç)', {
      fontSize: '12px', fontFamily: 'Arial, sans-serif', color: '#cc99ff', fontStyle: 'bold'
    }).setOrigin(0.5);
    ...
    if (this.recentSales.length > 0) {
      ...
      this.add.text(400, bbY - 10, 'Son Satışlar (Tıkla → Geri Al)', {
        fontSize: '10px', fontFamily: 'Arial, sans-serif', color: '#AAAA66'
      }).setOrigin(0.5);
    ...
    this.tooltipTxt = this.add.text(400, 552, 'Eşyaların üzerine gel → bilgi gör', {
      fontSize: '11px', fontFamily: 'Arial, sans-serif', color: '#666', align: 'center'
    }).setOrigin(0.5);
  ...
  showQtyInput(x, y, maxQty, callback) {
    ...
    const okBtn = this.add.text(360, 320, 'Tamam', {
      fontSize: '15px', fontFamily: 'Arial, sans-serif', color: '#90ee90',
      backgroundColor: '#1a3a1a', padding: { x: 14, y: 6 }, fontStyle: 'bold'
    }).setOrigin(0.5).setDepth(52).setInteractive({ useHandCursor: true });

    // Cancel button
    const cancelBtn = this.add.text(440, 320, 'İptal', {
      fontSize: '15px', fontFamily: 'Arial, sans-serif', color: '#ff6666',
  ...
  drawCrafting() {
    ...
    const exitBtn = this.add.text(400, 564, 'Çık [ESC]', {
      fontSize: '18px', fontFamily: 'Arial, sans-serif', color: '#ff8888',
